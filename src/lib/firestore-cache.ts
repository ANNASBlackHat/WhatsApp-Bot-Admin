/**
 * Cache-first Firestore read helpers.
 *
 * The web SDK is initialized with `persistentLocalCache` (see
 * `src/lib/firebase.ts`), so reads can be served instantly from the on-device
 * IndexedDB cache and reconciled with the server afterwards.
 *
