...
export default function SettingsPage({ params }: PageProps) {
  ...
  return (
    <main className="mx-auto max-w-4xl p-4 sm:p-6 space-y-6">
      {/* Header section */}
      <div>
        <h1 className="text-xl font-medium text-[#1C1C1A]">Account Settings</h1>
        <p className="text-xs text-[#6B6A62]">
          Manage global kill switch, quiet hours schedule, default policies, and prompt library for account <code className="font-mono text-[#1C1C1A] font-medium">{waId}</code>
        </p>
      </div>

      {/* 1. Global Master Kill Switch Section */}
      <div className="rounded-lg border border-[#B23B31]/30 bg-[#F5E4E1] p-5 shadow-xs">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-full bg-[#B23B31] animate-pulse" />
              <h2 className="text-sm font-semibold uppercase tracking-wider text-[#B23B31]">
                Global Master Kill Switch
              </h2>
            </div>
            <p className="text-xs text-[#1C1C1A] leading-relaxed">
              Bound to <code className="font-mono bg-[#FFFFFF] px-1 py-0.5 rounded text-[11px]">wa_bot/{waId}.is_bot_active</code>. Turning this OFF immediately stops AI auto-replies across <strong>ALL {chats.length} contacts</strong> on this account.
            </p>
          </div>
      ...
      <div className="rounded-lg border border-[#E7E5DD] bg-[#FFFFFF] p-5 shadow-xs space-y-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-[#E7E5DD] pb-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-base">🌙</span>
              <h2 className="text-sm font-medium text-[#1C1C1A]">Scheduled Quiet Hours</h2>
            </div>
            <p className="text-xs text-[#6B6A62]">
              Bound to <code className="font-mono bg-[#FAFAF8] px-1 py-0.5 rounded">wa_bot/{waId}.quiet_hours</code>. Pauses AI auto-replies across all contacts during specified hours.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-[#1C1C1A] focus:ring-offset-2 ${
                quietHoursEnabled ? "bg-[#2F7A5C]" : "bg-[#A6A499]"
              }`}
            >
              <span
                className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-[#FFFFFF] shadow-sm transition duration-150 ease-in-out ${
        ...
        <form onSubmit={handleSaveQuietHours} className="space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div>
              <label htmlFor="quiet-start-time" className="block text-xs font-medium text-[#1C1C1A] mb-1">
                Start Time (24h)
              </label>
              <input
                id="quiet-start-time"
                ...
                className="w-full rounded border border-[#E7E5DD] bg-[#FAFAF8] px-3 py-2 text-xs text-[#1C1C1A] focus:border-[#1C1C1A] focus:bg-[#FFFFFF] focus:outline-none focus:ring-1 focus:ring-[#1C1C1A]"
              />
            </div>

            <div>
              <label htmlFor="quiet-end-time" className="block text-xs font-medium text-[#1C1C1A] mb-1">
                End Time (24h)
              </label>
              <input
                id="quiet-end-time"
            ...
            <div>
              <label htmlFor="quiet-timezone" className="block text-xs font-medium text-[#1C1C1A] mb-1">
                Timezone
              </label>
              <input
                id="quiet-timezone"
          ...
          <div className="flex items-center justify-between pt-2">
            {quietSaveStatus ? (
              <span className="text-xs font-medium text-[#2F7A5C]">{quietSaveStatus}</span>
            ) : <span />}

            <button
              type="submit"
              disabled={updatingQuiet || loading}
              className="rounded bg-[#1C1C1A] px-4 py-2 text-xs font-medium text-[#FFFFFF] transition-colors hover:bg-[#333330] focus:outline-none focus:ring-2 focus:ring-[#1C1C1A] disabled:opacity-50"
            >
      ...
      <div className="rounded-lg border border-[#E7E5DD] bg-[#FFFFFF] p-5 shadow-xs">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-xs text-[#6B6A62]">
              Bound to <code className="font-mono bg-[#FAFAF8] px-1 py-0.5 rounded">wa_bot/{waId}.default_bot_active_for_new_contacts</code>. Sets whether first-time senders start with the bot enabled automatically.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-[#1C1C1A] focus:ring-offset-2 disabled:opacity-50 ${
                defaultPolicyActive ? "bg-[#2F7A5C]" : "bg-[#B9722F]"
              }`}
            >
              <span
                className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-[#FFFFFF] shadow-sm transition duration-150 ease-in-out ${
      ...
      <div className="rounded-lg border border-[#E7E5DD] bg-[#FFFFFF] p-5 shadow-xs flex items-center justify-between">
        <div>
          <h2 className="text-sm font-medium text-[#1C1C1A]">Prompt Library</h2>
          <p className="text-xs text-[#6B6A62]">
        ...
        <Link
          ...
          className="inline-flex items-center gap-1.5 rounded-md border border-[#E7E5DD] bg-[#FAFAF8] px-3.5 py-2 text-xs font-medium text-[#1C1C1A] transition-colors hover:bg-[#F3F2ED]"
        >
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1C1C1A]/40 p-4">
          <div className="w-full max-w-md rounded-lg border border-[#E7E5DD] bg-[#FFFFFF] p-6 shadow-lg space-y-4">
            <div className="flex items-center gap-2 text-[#B23B31]">
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="rounded bg-[#B23B31] px-4 py-2 text-xs font-medium text-[#FFFFFF] transition-colors hover:bg-[#962F26]"
              >
